...
func enter() -> void:
	...
	character.animation_player.animation_finished.connect(end_attack)
	audio.stream = attack_sound
	audio.pitch_scale = randf_range(0.9, 1.1)
	audio.play()
	attacking = true

# small delay before enabling the hitbox 
...
func exit() -> void:
	character.animation_player.animation_finished.disconnect(end_attack)
